...
export function matrix(grid: {x: number, y: number}, random: number[] | undefined = undefined): number[][]
{
    return matrixGenerate(grid, stripes, random);
}

function matrixGenerate(grid: IPoint, reel: number[][], random: number[] | undefined = undefined): number[][]
{
