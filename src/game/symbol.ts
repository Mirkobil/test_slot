...
export default class Symbol extends Container
{
    ...
    public load(): void {
        ...
        this.addChild(drawOutline({
            rect: new Rectangle(0, 0, this.option.spacedTileSize.x, this.option.spacedTileSize.y),
            width: 1,
            color: 'red',
            zIndex: 10
        }));
    ...
    public setState(state: state, force: boolean = false): TrackEntry | undefined {
    ...
    protected validAnimation(value: state): boolean {
        return this.spine.skeleton.data.animations.map(i => i.name).includes(value);
    }
